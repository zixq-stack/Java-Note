...
# [#](#Java的I/O演进之路) Java的I/O演进之路

## [#](#I/O 模型基本说明) I/O 模型基本说明
...
## [#](#BIO、NIO、AIO 适用场景) BIO、NIO、AIO 适用场景
...
# [#](#JAVA BIO深入剖析) JAVA BIO深入剖析

## [#](#Java BIO 基本介绍) Java BIO 基本介绍
...
## [#](#Java BIO模式下的端口转发思想) Java BIO模式下的端口转发思想
...
#### [#](#服务端接收登陆消息以及监测离线) 服务端接收登陆消息以及监测离线

> 目标：在上节我们实现了服务端可以接收多个客户端，然后服务端可以接收多个客户端连接，接下来我们要接收客户端的登陆消息
...
> 目标：在上节实现了接收客户端的登陆消息，然后提取当前在线的全部的用户名称和当前登陆的用户名称发送给全部在线用户更新自己的在线人数列表接下来要接收客户端发来的群聊消息推送给当前在线的所有客户端
...
> 目标：在上节我们接收了客户端发来的群聊消息推送给当前在线的所有客户端，接下来要解决私聊消息的推送逻辑
...
#### [#](#启动客户端界面 ,登陆，刷新在线) 启动客户端界面 ,登陆，刷新在线

> 目标：**启动客户端界面**，登陆，刷新在线人数列表
...
> 目标：客户端发送群聊消息，@消息，以及私聊消息

##### [#](#实现步骤) 实现步骤

* 客户端启动后，在聊天界面需要通过发送按钮推送群聊消息，@消息，以及私聊消息

![image-20200223232406727](https://img2023.cnblogs.com/blog/2421736/202405/2421736-20240506230301372-1320172868.png)
...
# [#](#JAVA NIO深入剖析) JAVA NIO深入剖析

在讲解利用NIO实现通信架构之前，我们需要先来了解一下NIO的基本特点和使用

## [#](#Java NIO 基本介绍) Java NIO 基本介绍
...
Java NIO的通道类似流，但又有些不同：既可以从通道中读取数据，又可以写数据到通道但流的（input或output)读写通常是单向的 通道可以非阻塞读取和写入通道，通道可以支持读取或写入缓冲区，也支持异步地读写
...
## [#](#NIO核心一：缓冲区(Buffer)) NIO核心一：缓冲区(Buffer)
...
## [#](#NIO核心二：通道(Channel)) NIO核心二：通道(Channel)

### [#](#通道Channe概述) 通道Channe概述
...
### [#](#FileChannel的常用方法) FileChannel的常用方法
...
### [#](#案例3-使用Buffer完成文件复制) 案例3-使用Buffer完成文件复制
...
### [#](#案例4-分散 (Scatter) 和聚集 (Gather)) 案例4-分散 (Scatter) 和聚集 (Gather)
...
```
### [#](#案例5-transferFrom()) 案例5-transferFrom()
...
## [#](#NIO核心三：选择器(Selector)) NIO核心三：选择器(Selector)

### [#](#选择器(Selector)概述) 选择器(Selector)概述
...
### [#](#选择器（Selector）的应用) 选择器（Selector）的应用
...
```
 ...
 当调用 register(Selector sel, int ops) 将通道注册选择器时，选择器对通道的监听事件，需要通过第二个参数 ops 指定可以监听的事件类型（用 可使用 SelectionKey  的四个常量 表示）：
...
## [#](#NIO非阻塞式网络通信原理分析) NIO非阻塞式网络通信原理分析

### [#](#Selector 示意图和特点说明) Selector 示意图和特点说明
...
* 4. 发送数据给服务端
	...
	sChannel.close();
	```
## [#](#NIO非阻塞式网络通信入门案例) NIO非阻塞式网络通信入门案例
...
## [#](#NIO 网络编程应用实例-群聊系统) NIO 网络编程应用实例-群聊系统

> 目标：**需求:进一步理解 NIO 非阻塞网络编程机制，实现多人群聊**
...
即可以理解为，read/write方法都是异步的，完成后会主动调用回调函数在JDK1.7中，这部分内容被称作NIO.2，主要在Java.nio.channels包下增加了下面四个异步通道：
...
- AIO方式使用于连接数目多且连接比较长（重操作）的架构，比如相册服务器，充分调用OS参与并发操作，编程比较复杂，JDK7开始支持Netty!
