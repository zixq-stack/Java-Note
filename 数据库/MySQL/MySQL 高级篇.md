...
- statement，记录的是 SQL的原文

好处：不需要记录每一行的变化，减少了 binlog 日志量，节约了 IO，提高性能。由于 SQL的执行是有上下文的，因此在保存的时候需要保存相关的信息，同时还有一些使用了函数之类的语句无法被记录复制。

 

- row：不记录 SQL语句上下文相关信息，仅保存哪条记录被修改

记录单元为每一行的改动，基本是可以全部记下来。但是由于很多操作，会导致大量行的改动（比如 alter table)，因此这种模式的文件保存的信息太多，日志量太大。 



- mixed，一种折衷的方案，普通操作使用 statement 记录，当无法使用 statement 的时候使用 row。 
