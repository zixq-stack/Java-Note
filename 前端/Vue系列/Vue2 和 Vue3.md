...
<img src="https://img2020.cnblogs.com/blog/2421736/202201/2421736-20220114223926258-537130399.png" alt="image" />
...
- 这里涉及到cookie的机制（后端理解就对照jwt单点登录流程原理）。
...
上面这样做之后，看起来好像成功了，但是运行一会就会发现：电脑的风扇会使劲儿转，而且要是在`change()`中输出一句话的话，会发现：执行了N多次，一直在跑^ _ ^ 。原因如下：
...
- 对上图中的内容做补充（蓝色的东西就是新加的）。下图这个图是为了先了解大概，接下来会简单说明一下图中的内容。
...
2. 每一个组件都调用了`VueComponent()`，但是：它们都不一样（源码中有嘛，每次都是创建了一个全新的sub，sub就是`VueComponent()`，最后把这个sub返回去了，验证一下咯。
...
但是：这里有一个有意思的东西，不了解原因的人很容易弄错，也是第一条说的组件就是指`VueComponent()`，从而会出现不了解的人认为：每个组件都是调了同一个`VueComponent()`，来看一下：

<img src="https://img2022.cnblogs.com/blog/2421736/202201/2421736-20220122155255489-880385657.png" alt="image" />



这两个长得一模一样，所以就会让人误会，玩Java的人看到这个肯定熟悉得不得了，这就是一个构造函数嘛，所以理解起来也更容易。
...
> xxx.vue文件，如果使用的是vscode+vetur，那么上面的模板可以用快捷键 ：就是输入 `<v`  然后回车就可以生成了。
...
<img src="https://img2022.cnblogs.com/blog/2421736/202201/2421736-20220125223949591-688189741.png" alt="image" />
...
<img src="https://img2022.cnblogs.com/blog/2421736/202201/2421736-20220126000605092-1366648685.png" alt="image" />



<img src="https://img2022.cnblogs.com/blog/2421736/202201/2421736-20220126000658310-1096365650.png" alt="image" />
...
<img src="https://img2022.cnblogs.com/blog/2421736/202201/2421736-20220126001042814-1082990613.png" alt="image" />



如果二者不同，那么就会造成二者中定义的都会被调用

<img src="https://img2022.cnblogs.com/blog/2421736/202201/2421736-20220126001342232-448333417.png" alt="image" />



<img src="https://img2022.cnblogs.com/blog/2421736/202201/2421736-20220126001316150-975655428.png" alt="image" />
...
- PS：不用前面玩的子传父，但是：自行可以先回顾一下子传父的传统方式：父给子一个函数，子执行，参数即为父要得到的数据。
...
> 实现方式：
>
> 1. vm + `beforeCreate()`【ps：初始化嘛，让关系在一开始就建立】；
> 2. `mounted()` + ` $on()` + `$emit()` + `beforeDestroy()`【ps：做收尾工作，解绑自定义事件 】+ `$off()`。
...
> 另外：从此处开始，可以先摸索饿了么的 [Element-ui 组件库](https://element.eleme.cn/#/zh-CN/component/changelog) 了，这是专门搭配Vue来做页面的网站 和 bootstrap一个性质，这个东西后续要用。@紫邪情
...
但是还有一种模式是：replace模式，这种模式是产生一个记录之后，就把上一次的记录给干掉了，所以效果就是不会有回退的记录，回退按钮都点不了【 PS：就是走一路断一路，没后路了 ^ _ ^ ，要实现这种模式也简单，就是：在router-link中加一个replace属性即可 】。
...
![image-20240215184040236](https://img2023.cnblogs.com/blog/2421736/202402/2421736-20240215184010709-1543432612.png)
...
![image-20240215184523631](https://img2023.cnblogs.com/blog/2421736/202402/2421736-20240215184453480-609986653.png)
...
- setup执行时机：在beforeCreate之前执行一次，同时在setup(){ }中的this是undefined
...
- `ref()`函数中接收的数据可以是：基本类型、也可以是对象类型。
  - ref函数修饰的是基本类型时【PS：即直接用 `let name = ref('紫邪情')` 】，则：数据代理就是Object.dinfineProperty的setter和getter。
...
![image-20240215190349673](https://img2023.cnblogs.com/blog/2421736/202402/2421736-20240215190320092-357550333.png)
...
经过上图的查看之后，其实也就知道Reflect该怎么玩了，调对应的API就可以了【PS：ECMA组织正打算把常用的一些API放到Reflect对象身上，如：目前把 `Object.defineProperty()` 就放在Reflect中了  - vue2的数据代理原理的API 】
...
1. 配置项写法：和name、setuo()保持平级，写法就是：按照官网中说的哪些名字直接写即可。
